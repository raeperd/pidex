...
- Valibot validates protocol boundaries, Effect Schema validates persisted metadata, native Node HTTP provides the server transport and SSE-compatible event streams, and Drizzle ORM over `node:sqlite` stores Pidex metadata while Pi JSONL remains the transcript source of truth.
...
- Electron main supervises one bundled child process that runs the host and its HTTP/SSE-compatible server. It passes private bootstrap data, waits for readiness, captures logs, restarts unexpected exits, and terminates the child on Quit. Pidex does not install an independently managed daemon or service.
...
- Large tool results are collapsed and fetched in bounded chunks. Unsupported or binary content is shown as bounded metadata rather than sent unbounded through the event stream.
...
- HTTP serves the web client, public host descriptor, pairing exchange, authenticated bounded resources, commands, and an SSE-compatible event stream for snapshots and live events.
- The descriptor and event-stream input negotiate a schema-validated protocol version. Desktop, host, and host-served mobile client ship together.
...
- Disabling remote access rejects new remote authentication, closes remote event streams, and removes the route only when it still matches the stored Pidex-owned mapping. Otherwise Pidex leaves Tailscale configuration untouched and reports manual cleanup.
...
- Live event streams use authenticated HTTP requests with CSRF protection, so long-lived credentials never appear in URLs.
- A paired mobile client may operate sessions in registered projects but cannot perform desktop-only administration, trust, or setting changes. Revocation blocks future requests and closes that client's active event streams.
...
- Tests prove user-visible behavior through the highest public seam that can observe it. Shared session, authorization, and continuity behavior is tested through the host HTTP/SSE-compatible contract, including its public bootstrap endpoints; browser UX is tested through the production web client; Electron-only lifecycle behavior is tested through the packaged application. Tests do not assert Svelte component state, private Pi SDK objects, implementation-specific call counts, or metadata layout.
- Because the repository has no implementation prior art yet, the first delivery phase establishes one reusable host contract harness. It starts a real isolated host, connects clients over real HTTP and SSE-compatible event streams, and provides only public controls for time, network loss, runtime outcomes, and host restart.
- Protocol tests use the real pinned Pi SDK at the documented runtime boundary. Non-inference cases run by default where supported, while inference-dependent coverage remains an explicit opt-in real-provider smoke.
...
- Authentication tests cover remote access disabled by default, local desktop bootstrap origin restrictions, short-lived pairing grants that work once through the QR code or link, independently issued and revoked device credentials, CSRF and origin enforcement on event streams, rate-limited authentication attempts, and active-connection revocation. Canary secrets placed in test credentials must never appear in HTTP/SSE payloads, browser-visible errors, or normal and security logs.
...
- Prior work, including T3 Code, leaves useful design lessons: keep one host as the execution boundary, verify the final remote endpoint, use pairing only to create a durable client credential, restore state from authoritative snapshots, and treat live event delivery as recoverable. Pidex does not depend on T3 Code's code, APIs, protocols, data, or compatibility.
...
- After the host records a client revocation, that credential's next authenticated request is rejected and its active event stream closes within ten seconds; other clients remain connected.
...
- Large tool results are collapsed and fetched in bounded chunks. Binary or unsupported content is represented by bounded metadata and is never inserted as an unbounded event-stream payload.
...
- The desktop shows a short-lived, single-use QR code or copyable pairing link. The pairing grant stays in the verified MagicDNS URL fragment and rejects reuse. Successful pairing mints a unique revocable device credential, and long-lived credentials never appear in HTTP logs or event-stream URLs.
...
- Desktop lists each paired client and its last connection time. Revocation rejects that client's future requests and closes its active event stream without affecting other clients or local desktop use.
- Disabling remote access rejects new remote authentication, closes remote event streams, and removes only a Tailscale Serve mapping that Pidex can prove it created. An unowned mapping remains untouched and produces manual cleanup guidance.
