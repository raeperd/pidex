...
export const actionStatusSchema = z.enum(["accepted", "running", "completed", "cancelled", "failed", "interrupted"]);
...
export const workspaceSchema = z.object({ id: idSchema, path: z.string().max(4096), name: z.string().max(300), trusted: z.boolean().nullable(), protectedResourcesSkipped: z.boolean(), resourceDiagnostics: z.array(z.object({ level: z.enum(["warning", "error"]), message: z.string().max(1000) })).max(50), models: z.array(modelSchema), sessions: z.array(sessionSummarySchema), commands: z.array(z.object({ name: z.string(), description: z.string().optional() })) });
...
export const toolItemSchema = z.object({ type: z.literal("tool"), id: z.string().max(200), name: z.string().max(200), argumentSummary: z.string().max(1000), state: z.enum(["running", "success", "error"]), preview: z.string().max(16_384), truncated: z.boolean(), resourceId: idSchema.optional(), outputSize: z.number().int().nonnegative().optional() });
...
export const chatSnapshotSchema = z.object({ chatId: idSchema, workspaceId: idSchema, sessionId: idSchema, sessionName: z.string().max(300).optional(), revision: z.number().int().nonnegative(), run: runOutcomeSchema.optional(), runStatus: runStatusSchema, model: z.string().max(300).optional(), thinkingLevel: thinkingLevelSchema, toolMode: toolModeSchema, activeTools: z.array(z.string().max(100)), items: z.array(transcriptItemSchema).max(200), transcriptStart: z.number().int().nonnegative(), transcriptTotal: z.number().int().nonnegative(), steeringQueue: z.array(z.string().max(20_000)), followUpQueue: z.array(z.string().max(20_000)), stats: statsSchema, extensionDialog: extensionDialogSchema.optional() });
...
export const serverEventSchema = z.discriminatedUnion("type", [
  ...
  eventBase.extend({ type: z.literal("run_status"), status: runStatusSchema, revision: z.number().int().nonnegative(), run: runOutcomeSchema.optional() }),
...
export const actionRequestSchema = z.object({ clientId: idSchema, actionId: idSchema, expectedRevision: z.number().int().nonnegative() });
const actionRequestBase = actionRequestSchema;
export const messageRequestSchema = actionRequestBase.extend({ text: z.string().min(1).max(20_000), delivery: z.enum(["normal", "steer", "follow-up"]), runId: idSchema.optional() });
export const abortRequestSchema = actionRequestBase.extend({ runId: idSchema });
export const acknowledgeInterruptedRequestSchema = actionRequestBase.pick({ clientId: true, actionId: true, expectedRevision: true });
export const configRequestSchema = actionRequestBase.extend({ model: z.string().max(300).optional(), thinkingLevel: thinkingLevelSchema.optional(), toolMode: toolModeSchema.optional() });
export const renameRequestSchema = actionRequestBase.extend({ name: z.string().trim().min(1).max(200) });
export const compactRequestSchema = actionRequestBase.extend({ instructions: z.string().max(4000).optional() });
export const dialogResponseSchema = actionRequestBase.extend({ requestId: idSchema, value: z.union([z.string().max(20_000), z.boolean(), z.null()]) });
export const toolOutputChunkSchema = z.object({ resourceId: idSchema, offset: z.number().int().nonnegative(), nextOffset: z.number().int().nonnegative(), total: z.number().int().nonnegative(), text: z.string().max(16_384), complete: z.boolean(), sourceTruncated: z.boolean() });
export const transcriptPageSchema = z.object({ items: z.array(transcriptItemSchema).max(100), start: z.number().int().nonnegative(), total: z.number().int().nonnegative() });
