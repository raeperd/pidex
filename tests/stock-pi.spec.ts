  ...
  test(`#139 stock Pi loads ${instructions}, excludes customization, and executes stock tools`, async ({}, testInfo) => {
    ...
    const provider = createServer((request, response) => {
      ...
      request.on("end", () => {
        ...
        response.write(
          `data: ${JSON.stringify({ id: "reply", object: "chat.completion.chunk", created: 1, model: "gpt-4.1", choices: [{ index: 0, delta, finish_reason: null }] })}\n\n`,
        );
        response.write(
          `data: ${JSON.stringify({ id: "reply", object: "chat.completion.chunk", created: 1, model: "gpt-4.1", choices: [{ index: 0, delta: {}, finish_reason: call ? "tool_calls" : "stop" }] })}\n\n`,
        );
