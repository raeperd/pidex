...
test("#137 Cancel Quit keeps work alive; Confirm awaits cancellation before both processes exit", async ({}, testInfo) => {
  ...
  try {
    ...
    expect((await page.evaluate(() => window.desktop.chooseProject()))?.runId).toBe(active?.runId);
