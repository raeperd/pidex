...
4. Verify and publish each layer as a draft.
   ...
   - Inspect the final diff and commit only intended changes after validation. Follow the [repository commit/title convention](../../../AGENTS.md), including titles generated by `gh-stack`. Create or update a draft PR; use `gh-stack` for dependent layers. Each PR targets its immediate parent so its diff contains only its own changes.
