...
This guide is based on the shared explorer `Schedule` module and its usage across `$EFFECT_REPO`.

Key source files:

- `$EFFECT_REPO/packages/effect/src/Schedule.ts`
- `$EFFECT_REPO/packages/effect/test/Schedule.test.ts`
...
The most common patterns in the shared explorer repo are:
