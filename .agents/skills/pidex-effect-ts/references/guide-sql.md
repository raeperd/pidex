...
This guide is based on the shared explorer Effect SQL modules in `$EFFECT_REPO`.

Key source files:

- `$EFFECT_REPO/packages/effect/src/unstable/sql/SqlClient.ts`
- `$EFFECT_REPO/packages/effect/src/unstable/sql/Migrator.ts`
- `$EFFECT_REPO/packages/effect/src/unstable/sql/SqlResolver.ts`
- `$EFFECT_REPO/packages/effect/src/unstable/sql/SqlSchema.ts`
- `$EFFECT_REPO/packages/effect/src/unstable/sql/SqlModel.ts`
...
`SqlResolver` is one of the clearest examples of SQL and Schema integration in the shared explorer repo.
...
Important behavior in the shared explorer repo:
...
This matches the model used by the shared explorer migrator implementation:
...
From the shared explorer packages, this is implemented as `Layer.effectDiscard(run(options))`.
...
The shared explorer SQL modules already integrate with spans and transaction context.
