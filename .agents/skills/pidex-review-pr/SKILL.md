...
description: Resolve Codex and independent subagent reviews and mark verified PRs ready. Use when pidex-implement-issue reaches review or the user requests PR review and readiness.
---

Run directly with `$pidex-review-pr <PR URL>` or as the review phase of `pidex-implement-issue`. Either request includes the Codex review request, review fixes, commits/pushes, replies, thread resolution, and readiness unless the user narrows the scope. Merge only when explicitly requested.
