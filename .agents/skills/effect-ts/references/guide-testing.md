...
This guide is based on the shared explorer `@effect/vitest` package in `$EFFECT_REPO`.

Key source files:

- `$EFFECT_REPO/packages/vitest/src/index.ts`
- `$EFFECT_REPO/packages/vitest/src/internal/internal.ts`
- `$EFFECT_REPO/packages/vitest/test/index.test.ts`
- `$EFFECT_REPO/packages/vitest/typetest/index.tst.ts`
...
The shared explorer repo also uses `expect` in some tests, but for skill guidance prefer `assert` in Effect-based tests because it keeps tests more uniform and explicit inside Effect programs.
...
Examples from the shared explorer tests show these are first-class parts of the API.
...
The shared explorer tests explicitly verify resource release through `afterAll`.
