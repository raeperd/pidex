...
This guide is based on the shared explorer Effect source in `$EFFECT_REPO`.

Key source files:

- `$EFFECT_REPO/packages/effect/src/Effect.ts`
- `$EFFECT_REPO/packages/effect/src/Tracer.ts`
- `$EFFECT_REPO/packages/effect/src/Logger.ts`
- `$EFFECT_REPO/packages/effect/src/Metric.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/NodeSdk.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/Tracer.ts`
...
The shared explorer Effect repo itself uses `fnUntraced` in a number of low-level internals and integration helpers. That does not make it the default recommendation for downstream application or business code.
...
The shared explorer repo provides `@effect/opentelemetry` layers such as:
...
The shared explorer repo includes a real integration layer for the OpenTelemetry JavaScript ecosystem in `@effect/opentelemetry`.
...
- `$EFFECT_REPO/packages/opentelemetry/src/NodeSdk.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/Tracer.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/Metrics.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/Logger.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/Resource.ts`
- `$EFFECT_REPO/packages/opentelemetry/src/WebSdk.ts`
...
From the shared explorer source, it accepts a configuration that can include:
...
Important integration points from the shared explorer source:
...
Important details from the shared explorer implementation:
...
The shared explorer layers use scoped acquisition and release for tracer providers, metric readers, and logger providers.
...
The shared explorer tracer module provides:
