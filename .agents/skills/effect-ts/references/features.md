...
Public package and module surface area to be aware of when researching or implementing a solution. Each module entry includes its shared explorer repo path so it can be located quickly.
...
- This explorer checkout has no public `src/index.ts` barrel or barrel exports.
