...
let restartCount = 0;
const logs: string[] = [];
const remember = (chunk: Buffer) => { for (const line of chunk.toString("utf8").split("\n").filter(Boolean)) logs.push(line.slice(0, 2000)); if (logs.length > 200) logs.splice(0, logs.length - 200); };

function spawnServer() {
  if (process.env.PIDEX_WEB_URL || quitting) return;
  const entry = path.resolve(import.meta.dirname, "../../server/dist/main.js");
  const child = spawn(process.execPath, [entry], { cwd: path.resolve(import.meta.dirname, "../../.."), env: { ...process.env, ELECTRON_RUN_AS_NODE: "1", PORT: String(port) }, stdio: ["ignore", "pipe", "pipe"] });
  serverChild = child;
  child.stdout?.on("data", remember); child.stderr?.on("data", remember);
  child.once("exit", () => { if (serverChild === child) serverChild = undefined; if (!quitting) { const wait = Math.min(5000, 300 * 2 ** restartCount++); setTimeout(spawnServer, wait); } });
}
async function waitForServer() {
  for (let attempt = 0; attempt < 80; attempt++) { try { const response = await fetch(`${localUrl}/api/health`); const health = healthSchema.safeParse(await response.json()); if (response.ok && health.success) return; } catch {} await new Promise((resolve) => setTimeout(resolve, 125)); }
