...
describe("HTTP control surface", () => {
  ...
  const mutate = (route: string, body: unknown, method = "POST") => fetch(`${origin}${route}`, { method, headers: { "Content-Type": "application/json", "X-Pidex-CSRF": csrf }, body: JSON.stringify(body) });
  ...
  async function waitForIdle(chatId: string) {
    for (let attempt = 0; attempt < 60; attempt++) {
      const snapshot = await (await fetch(`${origin}/api/chats/${chatId}`)).json() as ChatSnapshot;
  ...
  it("enforces Host, CSRF, request limits, and completes the fake flow", async () => {
    const badHostStatus = await new Promise<number>((resolve, reject) => {
      ...
      const request = http.request({ hostname: target.hostname, port: target.port, path: "/api/health", headers: { Host: "evil.example" } }, (response) => { response.resume(); resolve(response.statusCode ?? 0); });
      request.on("error", reject); request.end();
    });
    expect(badHostStatus).toBe(403);
    const noCsrf = await fetch(`${origin}/api/workspaces/open`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: process.cwd() }) });
    expect(noCsrf.status).toBe(403);
    const huge = await mutate("/api/workspaces/open", { path: "x".repeat(70_000) });
    expect(huge.status).toBe(413);
    const chat = await createChat();
    const sent = await mutate(`/api/chats/${chat.chatId}/messages`, { clientId, actionId: "actionsecurity01", expectedRevision: chat.revision, text: "hello", delivery: "normal" });
  ...
  it("accepts a prompt once, replays its outcome, rejects conflicts, and stops only the exact run", async () => {
    const chat = await createChat();
    const body = { clientId, actionId: "actionreplay001", expectedRevision: 0, text: "stop this", delivery: "normal" };
    ...
    expect(await replay.json()).toMatchObject({ actionId: body.actionId, runId: accepted.runId, replayed: true });
    const live = await (await fetch(`${origin}/api/chats/${chat.chatId}`)).json() as ChatSnapshot;
    expect(live.items.filter((item) => item.type === "user" && item.text === "stop this")).toHaveLength(1);

    const conflict = await mutate(`/api/chats/${chat.chatId}/messages`, { ...body, text: "different work" });
    expect(conflict.status).toBe(409);
    expect(await conflict.json()).toMatchObject({ error: { code: "action_conflict" } });
    const stale = await mutate(`/api/chats/${chat.chatId}/messages`, { ...body, actionId: "actionstale0001", text: "new work" });
    expect(stale.status).toBe(409);
    expect(await stale.json()).toMatchObject({ error: { code: "stale_revision" } });

    const wrongStop = await mutate(`/api/chats/${chat.chatId}/abort`, { clientId, actionId: "actionstopwrong", expectedRevision: 1, runId: "wrongrun1234" });
    expect(wrongStop.status).toBe(409);
    expect(await wrongStop.json()).toMatchObject({ error: { code: "run_mismatch" } });
    const stopped = await mutate(`/api/chats/${chat.chatId}/abort`, { clientId, actionId: "actionstopright", expectedRevision: 1, runId: accepted.runId });
    expect(stopped.status).toBe(202);
    expect(await stopped.json()).toMatchObject({ runId: accepted.runId, revision: 2, status: "completed" });
    const settled = await waitForIdle(chat.chatId);
    expect(settled.run).toMatchObject({ runId: accepted.runId, status: "cancelled", requiresAcknowledgement: false });
  });

  it("keeps large tool output out of events and serves it in bounded chunks", async () => {
    const chat = await createChat();
    const sent = await mutate(`/api/chats/${chat.chatId}/messages`, { clientId, actionId: "actionlargetool", expectedRevision: 0, text: "LARGE_TOOL", delivery: "normal" });
    expect(sent.status).toBe(202);
    const settled = await waitForIdle(chat.chatId);
    ...
    let offset = 0; let assembled = ""; let complete = false;
    while (!complete) {
      const response = await fetch(`${origin}/api/chats/${chat.chatId}/tools/${tool!.resourceId}?offset=${offset}&limit=4096`);
      expect(response.status).toBe(200);
      const chunk = await response.json() as ToolOutputChunk;
      expect(chunk.text.length).toBeLessThanOrEqual(4096);
      assembled += chunk.text; offset = chunk.nextOffset; complete = chunk.complete;
    }
  ...
  it("bounds authoritative transcripts and pages older items", async () => {
    ...
    for (let index = 0; index < 240; index++) record.items.push({ type: "notice", id: `history-${index}`, level: "info", text: `Historical event ${index}` });
    const bounded = await (await fetch(`${origin}/api/chats/${chat.chatId}`)).json() as ChatSnapshot;
    ...
    const response = await fetch(`${origin}/api/chats/${chat.chatId}/transcript?before=${bounded.transcriptStart}&limit=50`);
    expect(response.status).toBe(200);
    ...
    expect((await fetch(`${origin}/api/chats/${chat.chatId}/transcript?before=20&limit=101`)).status).toBe(400);
  });
});
