...
import type { AdapterEvent, AdapterSession, AdapterSessionInfo, AdapterWorkspaceInfo, PiAdapter } from "./adapter.js";
...
class FakeSession implements AdapterSession {
  ...
  constructor(private readonly cwd: string, info?: AdapterSessionInfo) {
    ...
    if (!info) {
      const record: AdapterSessionInfo = { id: this.nativeId, nativeId: this.nativeId, nativePath: this.nativePath, firstMessage: "New session", createdAt: new Date().toISOString(), modifiedAt: new Date().toISOString(), messageCount: 0 };
  ...
  async prompt(text: string): Promise<void> {
    ...
    this.isIdle = false; this.stopped = false;
    const now = Date.now();
    const user: TextItem = { type: "user", id: `u-${now}`, text, complete: true, timestamp: new Date(now).toISOString() };
    this.messages.push(user); this.emit({ type: "message", item: user });
    if (text === "DIALOG") {
      const id = randomUUID().replaceAll("-", "");
      const answer = await new Promise<string | boolean | null>((resolve) => { this.dialogResolve = resolve; this.emit({ type: "dialog", dialog: { id, kind: "confirm", title: "Extension confirmation", message: "Continue the deterministic extension?" } }); });
      this.dialogResolve = undefined; this.emit({ type: "dialog" }); this.emit({ type: "notice", level: answer === true ? "info" : "warning", text: answer === true ? "Extension confirmed." : "Extension cancelled safely." });
    }
    ...
    for (const chunk of chunks) {
      ...
      await delay(80); assistant.thinking = `${assistant.thinking ?? ""}checking `; this.emit({ type: "delta", itemId: assistant.id, delta: "checking ", channel: "thinking" });
      await delay(80); assistant.text += chunk; this.emit({ type: "delta", itemId: assistant.id, delta: chunk, channel: "text" });
    }
    if (!this.stopped) {
  ...
  async steer(text: string) { if (this.isIdle) throw new Error("Steer is only available during a run"); this.steering.push(text); this.emit({ type: "queue", steering: [...this.steering], followUp: [...this.followUps] }); }
  async followUp(text: string) { if (this.isIdle) throw new Error("Follow-up is only available during a run"); this.followUps.push(text); this.emit({ type: "queue", steering: [...this.steering], followUp: [...this.followUps] }); }
  async abort() { this.stopped = true; this.clearQueue(); }
  clearQueue(kind: "steering" | "follow-up" | "all" = "all") { if (kind !== "follow-up") this.steering = []; if (kind !== "steering") this.followUps = []; this.emit({ type: "queue", steering: [...this.steering], followUp: [...this.followUps] }); }
  async configure(input: { model?: string; thinkingLevel?: AdapterSession["thinkingLevel"]; toolMode?: "read-only" | "full" }) { if (!this.isIdle) throw new Error("Configuration can only change while idle"); if (input.model) this.model = input.model; if (input.thinkingLevel) this.thinkingLevel = input.thinkingLevel; if (input.toolMode) this.activeTools = input.toolMode === "read-only" ? ["read", "grep", "find", "ls"] : ["read", "bash", "edit", "write", "grep", "find", "ls"]; }
  rename(name: string) { this.sessionName = name; const record = saved.get(this.cwd)?.find((entry) => entry.nativeId === this.nativeId); if (record) record.name = name; }
  async compact() { if (!this.isIdle) throw new Error("Compact is only available while idle"); this.emit({ type: "notice", level: "info", text: "Session compacted by deterministic adapter." }); }
  getStats() { return { messages: this.messages.length, toolCalls: Math.floor(this.messages.length / 2), tokens: this.messages.reduce((n, m) => n + Math.ceil(m.text.length / 4), 0), cost: 0 }; }
  respondToDialog(_requestId: string, value: string | boolean | null) { if (!this.dialogResolve) throw new Error("No extension dialog is pending"); this.dialogResolve(value); }
  dispose() { this.stopped = true; this.dialogResolve?.(null); this.dialogResolve = undefined; this.listeners.clear(); }
}
