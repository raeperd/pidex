 ...
 * the transition is durable — the status to record. A missing `durable` key is a statement
 * rather than an omission: a stop request and both ends of a compaction change what clients see
 * without ever reaching disk, which is why no persisted run has ever held those values.
 */
