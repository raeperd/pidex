...
describe("local security", () => {
  it("rejects path-prefix traps and symlink escapes", async () => {
    const base = await mkdtemp(path.join(os.tmpdir(), "pidex-path-"));
    ...
    await expect(canonicalWorkspace(outside, [canonicalRoot])).rejects.toMatchObject({
      status: 403,
    });
  });
  it("validates ports and redacts obvious bearer secrets", () => {
    expect(parsePort("4783")).toBe(4783);
    expect(() => parsePort("80")).toThrow(/1024/);
